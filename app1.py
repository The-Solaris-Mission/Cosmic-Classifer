...
def load_model():
    with open('model/best_rf_model_conf.pkl', 'rb') as file:
        model = pickle.load(file)
    return model

# Load the model
best_rf_model_conf = load_model()
...
if st.button('Predict'):
    if input_data.isnull().values.any():
        ...
        prediction = best_rf_model_conf.predict(input_data)
        confidence = best_rf_model_conf.predict_proba(input_data)
